...
class NotificationAPIRepository  @Inject constructor(
    ...
    // Mark Read
    suspend fun markReadNotification(token: String, notiId: Int): Resource<NotiUpdateRespone> {
        return try {
            val response = notificationAPIService.markReadNotification("Bearer $token", notiId)
            if (response.isSuccessful) {
                Resource.success(response.body())
            } else {
                val errorMessage = ErrorHandler.parseError(response.errorBody())
    ...
    // Mark All Read
    suspend fun markAllReadNotification(token: String, userId: Int): Resource<Boolean> {
        return try {
            val response = notificationAPIService.markAllReadNotification("Bearer $token", userId)
            if (response.isSuccessful) {
                Resource.success(response.body())
            } else {
                val errorMessage = ErrorHandler.parseError(response.errorBody())
